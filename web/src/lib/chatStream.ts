...
export function sendMessageFrame(content: string, idempotencyKey: string): string {
...
export function chatStreamReducer(
  ...
  switch (action.type) {
    ...
    case "completed": {
      return state && state.status === "streaming" ? { ...state, status: "completed" } : state;
    }
    case "failed": {
      return state
...
export function errorCodeFromFrame(frame: Record<string, unknown>): string | null {
